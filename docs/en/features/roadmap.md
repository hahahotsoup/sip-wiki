...
- 🌐 **WebUI / HTTP gateway**: develop a separate external C# program that maps sip's CLI capability to HTTP (accessible over the public internet); it lives in the same directory as sip and uses a WebUI to lower the TUI entry barrier — easy to use while keeping security tight
