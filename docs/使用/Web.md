...
- **本地优先**：默认只监听 `127.0.0.1`，数据不出机器
- **防注入**：所有参数以列表形式传给子进程（不经 shell），命令走白名单
- **超时保护**：全部 CLI 调用有超时上限，避免卡死
- **遵守 sip 的安全边界**：`sip --init`（录入 API Key）仍需在真实终端手动执行，Web 界面不会代跑；全文抓取沿用 sip 的 SSRF 防护
